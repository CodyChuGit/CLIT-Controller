  ...
  return (
    <div className="card flex flex-col p-4">
      ...
      <dl className="mb-3 space-y-1 text-xs text-neutral-600 dark:text-neutral-400">
        ...
        <div className="flex justify-between gap-2">
          <dt className="text-neutral-400">Auth</dt>
          <dd className="truncate text-right">{p.authMode}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-neutral-400">Usage mode</dt>
          <dd className="truncate text-right">{p.usageMode}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-neutral-400">Preferred use</dt>
          <dd className="truncate text-right" title={p.preferredUse}>{p.preferredUse}</dd>
        </div>
        <div className="flex items-center justify-between gap-2">
          <dt className="text-neutral-400">Usage health</dt>
          <dd className="flex items-center gap-2">
            {p.usageHealth ? <UsageHealthBadge value={p.usageHealth} /> : <span>—</span>}
            {p.callsToday > 0 && <span className="text-neutral-400">{p.callsToday} calls</span>}
          </dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-neutral-400">Last checked</dt>
          <dd>{p.lastChecked ? new Date(p.lastChecked).toLocaleTimeString() : "never"}</dd>
        </div>
      </dl>
