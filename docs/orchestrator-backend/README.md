...
orchestrator backend for AgentFlow Studio.
...
- Directive-based orchestration through `agentflow-task`, `agentflow-queue`,
...
a durable orchestration system with explicit state, resumable execution, stronger
...
- [01 Target Capability](./01-target-capability.md) defines what "full functionality"
  means for the orchestrator backend.
...
- Keep AgentFlow local-first. The backend shells out to user-owned CLIs and stores
...
- Add stricter machine state beside the markdown. The current JSON files are useful,
  but full orchestration needs a durable run ledger, explicit state transitions, and
  restart recovery.
- Treat the orchestrator as a policy-bound decision maker, not an unrestricted shell.
...
| Orchestrator chat | `backend/agentflow/chat_service.py` | CLI-backed chat and directive parsing |
